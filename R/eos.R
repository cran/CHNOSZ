...
# 20060729 eos.args(), cgl() # 11/17/03 hkf.R jmd
...
cgl <- function(property=NULL,T=298.15,P=1,ghs=NULL,eos=NULL) {
  # calculate properties of crystalline, liquid (not H2O) and gas species
  # argument handling
  ...
  for(k in 1:nrow(ghs)) {
    ...
    for(i in 1:length(prop)) {
      ...
      if(property=='g') {
        ...
        p[is.na(p)] <- 0
        p <- GHS$G + p
        #if(!is.na(p[1])) p <- GHS$G + p else p <- rep(GHS$G,length.out=length(T))
      ...
      if(property=='h') { 
        ...
        p[is.na(p)] <- 0
        p <- GHS$H + p
        #if(!is.na(p[1])) p <- GHS$H + p else p <- rep(GHS$H,length.out=length(T))
        if(!is.na(EOS$f) & !is.na(EOS$lambda)) if(EOS$f!=0) {
      ...
      if(property=='s') {
        ...
        p[is.na(p)] <- 0
        p <- GHS$S + p
        #if(!is.na(p[1])) p <- GHS$S + p else p <- rep(GHS$S,length.out=length(T))
        if(!is.na(EOS$f) & !is.na(EOS$lambda)) if(EOS$f!=0) {
